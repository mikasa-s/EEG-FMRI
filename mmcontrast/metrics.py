...
def contrastive_retrieval_metrics(eeg_embed: torch.Tensor, fmri_embed: torch.Tensor) -> dict[str, float]:
    ...
    eeg_r1 = (eeg_to_fmri_rank[:, 0] == targets).float().mean().item()
    fmri_r1 = (fmri_to_eeg_rank[:, 0] == targets).float().mean().item()
    eeg_r5 = (eeg_to_fmri_rank[:, : min(5, similarity.size(1))] == targets.unsqueeze(1)).any(dim=1).float().mean().item()
    fmri_r5 = (fmri_to_eeg_rank[:, : min(5, similarity.size(0))] == targets.unsqueeze(1)).any(dim=1).float().mean().item()
